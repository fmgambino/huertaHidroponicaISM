...
Deno.serve(async request=>{
  ...
  try{
    ...
    if(!expected)return json({error:'DEVICE_ENROLLMENT_KEY no configurada en Supabase Secrets',code:'ENROLLMENT_NOT_CONFIGURED'},503);
    if(request.headers.get('x-enrollment-key')!==expected)return json({error:'Credencial de fabricación inválida: no coincide con Supabase Secrets',code:'ENROLLMENT_KEY_MISMATCH'},401);
    ...
    if(body.action==='telemetry'){
      ...
      const allowed=['air_temperature','humidity','water_temperature','ph','tds_ppm','cpu_temperature'];
      for(const key of allowed)if(body[key]!=null&&(!Number.isFinite(body[key])||Math.abs(body[key])>100000))return json({error:'Medición inválida'},400);
